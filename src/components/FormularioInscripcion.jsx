...
export const FormularioInscripcion = () => {
    ...
    const handleSelectedCursoChange = async (event) => {
        ...
        try {
            ...
            limpiarFormDesde("datosDeInscripcion")
    ...
    const handleErroresEnvioFormulario = () => {
        ...
        if (isNaN(startDateCurso) || startDateCurso === null) throw new Error('Es necesario indicar una fecha inicio del curso')
        if (isNaN(endDateCurso) || endDateCurso === null) throw new Error('Es necesario indicar una fecha fin del curso')
        if (isNaN(startDateInscripcion) || startDateInscripcion === null) throw new Error('Es necesario indicar una fecha inicio de inscripción del curso')
        if (isNaN(endDateInscripcion) || endDateInscripcion === null) throw new Error('Es necesario indicar una fecha fin de inscripción del curso')
        if (!Number.isInteger(Number(cupo)) || cupo <= 0) throw new Error('El cupo debe ser un entero mayor a cero')
        if (!Number.isInteger(Number(cantidadHoras)) || cantidadHoras <= 0) throw new Error('La cantidad de horas debe ser un valor entero mayor que cero')
                        ...
                        <div className="detalle-inscripcion">
                            <div className="fecha-cupo-cantidad">
                                ...
                                <select className="form-select" id="medioInscripcion" value={selectedMedioInscripcion} onChange={handleMedioInscripcionChange}>
                                    <option value="sin-seleccionar-medio-inscripcion">Seleccionar</option>
                                    {
